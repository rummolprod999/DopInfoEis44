...
type AbstractParserFtpEis() =
    ...
    member __.GetListArrays(pathParse : string, s : S.FtpUser) =
        ...
        while wh do
            try
                ...
                wh <- false
                if count > 1 then Logging.Log.logger $"Удалось получить список архивов после попытки %d{count}"
                ftp.Disconnect()
            with
                | ex when ex.Message.Contains("Failed to change directory") -> Logging.Log.logger $"Не смогли найти директорию %s{pathParse}"
                                                                               wh <- false
                | e when count > 3 -> Logging.Log.logger $"Не смогли найти директорию после попытки %d{count}"
    ...
    member __.GetArch(arch : string, pathParse : string, s : S.FtpUser) =
        ...
        while wh do
            try
                ...
                wh <- false
                if count > 1 then Logging.Log.logger $"Удалось скачать архив после попытки %d{count}"
                ftp.Disconnect()
            with
                | ex when count > 50 -> Logging.Log.logger $"Не смогли скачать архив после попытки %d{count}"
    ...
    member __.Unzipper(file : FileInfo) =
        ...
        try
            ...
            file.Delete()
        with ex -> Logging.Log.logger $"Не удалось извлечь файл %s{file.Name} %s{ex.Message}"
                   try
                       let proc = new Process()
                       proc.StartInfo <- ProcessStartInfo("unzip", String.Format("-B {0} -d {1}", file.FullName, dir.Name))
                       proc.Start() |> ignore
                       proc.WaitForExit()
                       Logging.Log.logger $"Извлекли файл альтернативным методом %s{file.Name}"
                       ()
                   with e -> Logging.Log.logger $"Не удалось извлечь файл альтернативным методом %s{file.Name} %s{ex.Message}"
        dir
